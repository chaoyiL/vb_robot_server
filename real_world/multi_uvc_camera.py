...
class MultiUvcCamera:
    def __init__(self,
        ...
        camera_format = repeat_to_list(
            camera_format, n_cameras, str) if camera_format is not None else ["MJPG"] * n_cameras
        auto_exposure = repeat_to_list(
            auto_exposure, n_cameras, int) if auto_exposure is not None else [3] * n_cameras
        exposure = repeat_to_list(
            exposure, n_cameras, int) if exposure is not None else [400] * n_cameras
        auto_white_balance = repeat_to_list(
            auto_white_balance, n_cameras, int) if auto_white_balance is not None else [0] * n_cameras
        wb_temperature = repeat_to_list(
            wb_temperature, n_cameras, int) if wb_temperature is not None else [4600] * n_cameras
        brightness = repeat_to_list(
            brightness, n_cameras, int) if brightness is not None else [0] * n_cameras
        gain = repeat_to_list(
            gain, n_cameras, int) if gain is not None else [100] * n_cameras
        capture_timestamp_delay = repeat_to_list(
            capture_timestamp_delay, n_cameras, (int, float)) if capture_timestamp_delay is not None else [0.101] * n_cameras
